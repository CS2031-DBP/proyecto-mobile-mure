...
export default function Album() {
    ...
    useEffect(() => {
        async function fetchAlbum() {
            try {
                ...
                setError(`Failed to load album data: ${(err as Error).message}`);
            }
        }
    ...
    return (
        <ScrollView contentContainerStyle={{ padding: 16, backgroundColor: "#fff" }}>
            <TouchableOpacity onPress={() => navigation.goBack()} style={{ position: 'absolute', top: 32, left: 16, zIndex: 1 }}>
                <IconButton icon="arrow-left" size={24} />
            </TouchableOpacity>
            <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 16, marginTop: 40 }}>
                <Image
                    source={{ uri: album.coverImageUrl }}
                    style={{ width: 100, height: 100, borderRadius: 8 }}
                />
                <View style={{ marginLeft: 16, flex: 1 }}>
                    <Text style={{ fontSize: 24, fontWeight: "bold" }}>{album.title}</Text>
                    <Text style={{ fontSize: 18, color: "gray" }}>by {album.artistName}</Text>
					
                    <View style={{ flexDirection: "row", alignItems: "center" }}>
                        <View style={{ marginTop: 8 }}>
                            <Text style={{ fontSize: 14 }}>{album.songsCount} songs </Text>
                            <Text style={{ fontSize: 14 }}>Total Duration: {album.totalDuration}</Text>
                            <Text style={{ fontSize: 14 }}>Release Date: {album.releaseDate}</Text>
                        </View>
						<View style={{ flexDirection: "column", alignItems: "center" }}>
						<TouchableOpacity onPress={handleLike}>
                            <IconButton icon={liked ? "heart" : "heart-outline"} size={24} iconColor={liked ? "red" : "black"} />
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => openLink(album.spotifyUrl)}>
            ...
            <Text style={{ fontSize: 16, marginVertical: 0 }}>{album.description}</Text>
