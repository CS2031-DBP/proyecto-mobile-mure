...
export default function FriendList ({ friends, currentUser, onFriendPress, onAddFriend, onDeleteFriend }: FriendListProps) {
  return (
        ...
        friends.map((friend) => (
          ...
          >
            ...
            <View style={{ flex: 1, marginLeft: 16 }}>
              <Text style={{ fontSize: 16, fontWeight: "bold" }}>@{friend.nickname}</Text>
              <Text style={{ fontSize: 14, fontWeight: "bold" }}>{friend.name}</Text>
              <Text style={{ fontSize: 12, color: "gray" }}>{friend.birthDate}</Text>
            </View>
